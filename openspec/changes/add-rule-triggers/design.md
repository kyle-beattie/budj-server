...
read-only account projection, and a Secure Enclave public key that nothing yet
verifies. This change makes the product work.
...
```
  Akahu ──webhook──▶ verify signature ──▶ akahu_user_id → user_id
                                     ...
                                     INSERT pending_execution
                                          ...
                                          thin APNs push
                                                │
  user taps ──▶ app ──▶ GET challenge ──▶ sign in Secure Enclave (Face ID)
                                                │
                          POST approve { nonce, signature }  ← user JWT, RLS
                                                │
...
- Money moves only after a fresh biometric signature that commits to the amount.
...
approval is then an exact contract over a specific number, which is also what
makes the signature in E11 meaningful.
...
### E11. Approval requires an ES256 signature over a server challenge

```
  GET  /api/executions/:id/challenge → { nonce, expiresAt }   (2 minute TTL)
  POST /api/executions/:id/approve   { nonce, signature }
```

The signed payload is a canonical, unambiguous encoding of the execution id, the
nonce, and a digest of the effect list including every amount. The server
verifies against the device public key enrolled during onboarding.

This is the point of enrolling the key. Face ID that merely unlocks a
Keychain-held refresh token is a client-side gate — the server sees the same
bearer token whether a face was presented or not. Requiring a signature means a
stolen session cannot approve a payment, and binding the digest to the amounts
means a tampered request fails verification rather than paying a different
number.

Nonces are single-use and short-lived. Declining needs no signature: refusing to
...
a control in code review while providing none.
...
```
  pending_executions
    ...
    status, challenge_nonce, challenge_expires_at,
...
`(user_id, akahu_txn_id)`, single-use nonces. Four layers, and a concurrency test
that runs approve twice in parallel and asserts exactly one payment.
...
**Biometric key invalidation.** `.biometryCurrentSet` invalidates the key when a
face is enrolled, so a user can be left unable to approve anything. → The
re-enrolment path from `add-onboarding` must be reachable from the approval
screen's error state, not only from settings.
...
5. Approval: challenge, signature verification, then payment initiation.
   Deliberately last.
